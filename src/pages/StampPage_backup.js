...
function StampPage() {
  ...
  useEffect(() => {
    if (viewMode === 'map' && window.kakao && window.kakao.maps) {
      ...
      console.log('Adding markers for:', selectedCategory, currentData.length, 'places');
      
      currentData.forEach(place => {
        // 모든 마커를 일단 표시 (레벨 체크는 나중에)
  ...
  // 지도 레벨에 따른 마커 표시/숨김 처리
  useEffect(() => {
    if (map && viewMode === 'map') {
      // 기존 마커들을 모두 제거하고 다시 추가
      const currentData = getCurrentData();
